...
class WSM_Featured_Post extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'wsmfeatured-post', __( 'Web Savvy - Featured Posts', 'genesis' ), $widget_ops, $control_ops );
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if ( $instance['show_title'] )
				...
				if ( ! empty( $instance['show_title'] ) ) {
					...
					// On front page, want Featured Post titles to be h2 instead of p tags. TKT 773
					$title_tag = "p";
					if ( in_array( 'category-c-s-s-news', get_body_class() ) || is_front_page() ) $title_tag = "h2";

					if ( genesis_html5() )
						printf( '<%s class="entry-title"><a href="%s" title="%s">%s</a></%s>', $title_tag, get_permalink(), the_title_attribute( 'echo=0' ), get_the_title(), $title_tag );
					else
						printf( '<%s><a href="%s" title="%s">%s</a></%s>', $title_tag, get_permalink(), the_title_attribute( 'echo=0' ), get_the_title(), $title_tag );						
				}
